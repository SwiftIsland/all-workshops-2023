...
private enum PlayState: CaseIterable {
    case blur, shrink, playing, fadein
...
private struct PlayerAnimationValues {
...
private struct PrincipledSoundCarouselView: View {
    let Rows = [
        GridItem(.flexible()),
    ]
            ...
            ZStack {
                VStack {
                    ...
                    ZStack {
                        ...
                        .phaseAnimator(PlayState.allCases, trigger: selectedMood) { content, phase in
                            content
                                .scaleEffect(phase.scaleText)
                                .opacity(phase.opacityText)
                                .offset(y: phase.offsetText)
                        }
                    }
                    .opacity((selectedMood != nil) ? 1 : 0)
                    
                    .keyframeAnimator(
                        initialValue: PlayerAnimationValues(),
                        trigger: selectedMood
                    ) { content, value in
                        content
                        ...
                        KeyframeTrack(\.width) {
                            SpringKeyframe(300, duration: 0.6)
                            SpringKeyframe(340, duration: 0.2, spring: .bouncy)
                            SpringKeyframe(370, duration: 0.4, spring: .snappy)
                            SpringKeyframe(300, duration: 0.2, spring: .snappy)
                        }
                        
                        KeyframeTrack(\.height) {
                            SpringKeyframe(60, duration: 0.6)
                            SpringKeyframe(24, duration: 0.2, spring: .bouncy)
                            SpringKeyframe(30, duration: 0.4, spring: .snappy)
                            SpringKeyframe(60, duration: 0.2, spring: .snappy)
                        }
                    }
                    .clipped()
                    .padding()
